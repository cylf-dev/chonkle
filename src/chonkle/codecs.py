...
_SIGNATURES_DIR = Path(__file__).parent / "signatures" / "numcodecs"
...
def _load_native_signature(codec_id: str) -> dict[str, Any]:
    ...
    """
    sig_path = _SIGNATURES_DIR / f"{codec_id}.json"
    if not sig_path.exists():
        msg = (
            f"No native codec signature for {codec_id!r} (looked in {_SIGNATURES_DIR})"
        )
